...
export default function CourseCard({
  ...
  return (
    <div className="grid grid-cols-2 grid-rows-2 gap-4 p-8 mb-6 border-b border-black dark:border-white">
      {/* Course ID */}
      ...
      <button
        ...
        disabled={disabled}
        className="col-span-2 self-start mt-4 px-4 py-2 border-2 border-[var(--glass-color)] bg-[var(--glass-bg)] text-[var(--glass-color)] rounded-[var(--glass-radius)] cursor-pointer transition-colors duration-300 ease-in-out hover:bg-[var(--glass-color)] hover:text-white hover:dark:text-black hover:border-[var(--glass-bg)] disabled:opacity-50 disabled:cursor-not-allowed"
      >
        Delete Course
      </button>
    </div>
